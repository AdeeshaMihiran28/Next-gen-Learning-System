...
def _get_artifact_path_or_404(job: Job, artifact_key: ArtifactKey) -> Path:
    artifact_path = build_artifact_path(job.job_id, artifact_key)
    if not artifact_path.is_file():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Artifact not found")
    return artifact_path


def _load_json_artifact(path: Path) -> dict | list:
